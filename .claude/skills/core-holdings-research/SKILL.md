...
1. **披露摘要**：`filings` 列表（8-K/10-K/10-Q，含日期与 edgarUrl）。对最近 1-2 条
   ...
   **filings 全部/部分来自 yfinance 镜像**，报告须写明「未读原文」。
   ...
   ##### 📌 镜像元数据已被第二来源交叉核对（2026-09-14 对拍，结论**三段分开写**）

   拿 FinnHub `/stock/filing` 与现行 yfinance 镜像做过一次 A/B（六只核心名，回看 120 天，
   只比 8-K/10-Q/10-K）。**三条结论方向不同，必须分开陈述，不得压缩成一句「已核对」**：

   | | 结论 | 依据 |
   |---|---|---|
   | ✅ **一致性** | **已交叉核对** | 两边都看得见的 **21 条**实质性 filing，日期/表单 **21/21 逐字一致（100%）** |
   | ❌ **完整性** | **未验证，且已知两边互有遗漏** | FinnHub 漏掉 **5 条**近期实质性 filing（含 NVDA 09-03、MSFT 09-02、AMZN 09-09 这几条报告真正引用过的）；yfinance 反过来漏掉 FinnHub 有的 **6 条**（含 2 份 10-Q） |
   | ❌ **正文** | **仍未读** | **22 个 URL 全部读不到**：FinnHub 的 `reportUrl`/`filingUrl` **全指回 `sec.gov`**，本机 TLS reset 原样挡住 |

   **怎么用**：可靠度描述从笼统的「未读原文」细化为
   **「元数据经第二来源交叉核对一致 · 完整性未验证 · 正文未读」**。
   这是一次真实的可靠度提升，但**只提升了元数据那一格**。

   ⚠️ **不得因此弱化「未读原文」**：两家给的都是同一个不可达 sec.gov 的链接，
   换数据商绕不过本机网络层的硬约束。**任何单一 filings 来源都不完整**——
   两边互有遗漏这件事本身就该记住，别把"某来源没有"读成"该 filing 不存在"。

   ⚠️ **该对拍不构成接入 FinnHub 的理由**：验收标准是"能不能读到正文"，答案是 0/22。
   它作为 filings 备用源**已被证伪**，不要再提这条路。（FinnHub 的内部人交易等
   其它数据是否有用，是**另一个未验证的假设**，与本条无关。）
...
5. **今日择时建议 + 目标价位**（分两层，给到具体价位与股数）：
   - **底仓累积**（服务补满 `target_usd` + 摊低成本）。**配了 `policy` 时用「基线 + 加速器」，
     ...
     - **加速器（有条件）**：估值门 `price ≤ mid`（带 `EPS_ONEOFF_INFLATED` 时用
       `normalized_vs_mid`）**且** 结构门 `buy_point ∈ {b1,b2,b3}` 且 `stroke_confirmed=true`
       → 在基线之上**额外**加一个 `trigger_extra_tranche_usd`（`≤ floor` 可再加大）。
       **VIX 是加码器不是门**（`panic_accelerator=true` ⇒ tranche 可加大，`false` 不阻止任何一条）。
       ⚠️ **「回踩 200DMA」不是结构扳机**，理由见 §3.8 结构门那一节。
     ...
     - 未配 `policy` 时才回到旧口径：估值门 `price ≤ mid` 才建议加仓（`≤ floor` 加大 tranche），
       结构扳机**同上，仍是 `buy_point ∈ {b1,b2,b3}` 且 `stroke_confirmed=true`**，VIX≥25 为加速窗口。
       （旧口径与新口径的差别只在「估值门是闸门还是加速条件」，**结构门的定义两边完全相同**
       ——否则歧义只是从这里挪到那里。）
...
#### `guidance_status` 三态（2026-09-02 起，取代原先的二值判断）

`earnings_gate.guidance_status` 由预取层判定，**三态各对应完全不同的动作**：

| 状态 | 含义 | 判定依据 | 该做什么 |
|---|---|---|---|
| **`captured`** | 指引已录 | `basis` **或** `basis_appended` 里有 `guidance_*` 数值 | 无动作；报告直接引用，并算「指引 vs 一致预期」缺口 |
| **`unavailable`** | **永久不可得** | `basis_appended.guidance_available === false`（采集时**已读原文**确认公司披露惯例不给） | **无动作**。报告写「永久不可得」，**不得**写成「下次须补录」 |
| **`missing`** | **真流程缺口** | 以上皆不满足 | 下次该名财报后按下方动作采集 |

`guidance_captured`（bool）= `status == "captured"`，保留供旧读法使用。
标位对应 `GATES_GUIDANCE_UNAVAILABLE` / `GATES_NO_GUIDANCE`，**两者都是软标、都不进 integrity**。

⚠️ **2026-09-02 修的两处误报，读这块时务必记住**：
1. **指引常录在 `basis_appended` 而非 `basis`** —— 补录规则本身就规定「只补 basis，不动
   red/green/written_at」，故财报后采集的指引落在 `basis_appended`。原实现只扫 `basis`，
   于是 AMZN($197–202B)/META($61–64B) 明明 2026-08-29 已录，却连续多日被报成「缺指引」。
2. **「拿不到」与「没去拿」不是一回事** —— AAPL/MSFT 新闻稿结构性不含 outlook、
   GOOGL 结构性不给营收指引（三者均已读原文确认）。把永久约束报成「下次财报后须补录」，
   等于每季派一个**永远做不完的动作**，真正的缺口反而淹没在里面。
   **一个永远亮着的告警等于没有告警。**

当前实况（2026-09-02）：`captured` = NVDA / AMZN / META；`unavailable` = AAPL / MSFT / GOOGL；
**`missing` = 无 → 真实流程缺口数 0**。下一个采集时点：2026-10-28（GOOGL/MSFT/META）、10-29（AAPL/AMZN）、11-17（NVDA）。
...
若读原文后确认**该公司根本不给**（而非本次抓取失败），把这个判断写进
`basis_appended.guidance_available = false` + `guidance_note`（写明依据），
状态即转 `unavailable`、不再每季重复告警——**这条判断必须来自读过原文，不得靠推测。**
...
##### ⚠️ ③ 只认「缠论买点 + 定笔」——「回踩 200DMA」**不是**结构扳机（2026-09-05 定，用户裁定）

此前 §3 要素 5 把扳机写成「b1/b2/b3 **或回踩 200DMA** 或 VIX≥25」，与这里的形式化不一致。
**统一按本节定义**，两处已改齐。三条理由：

1. **200DMA 是一条均线，不是缠论结构。** 本项目 55% 主轴的信号本体只有分型 / 笔 / 中枢 /
   买卖点，「回踩均线」在缠论里**不产生任何信号**。把它塞进结构轴，等于在结构门上
   开一个非结构的后门——而三轴之所以是 AND，前提正是三根轴各自问一个**不同且成立**的问题。
2. **它会绕过右端护栏。** `stroke_confirmed` 是 A 定笔 / B 迟滞 / C' 波动率三层防护的落点；
   均线乖离**没有定笔概念**，「回踩 200DMA」可以在末笔尚未确认时就放行，
   恰好是护栏要拦的那类场景（右端笔被新 K 重画 ⇒ 信号隔夜翻转）。
3. **台账 policy 自己就没写这一支。** `core_ledger.json` 的 `build_schedule.rule` 原文是
   「扳机(price<=mid + b1/b2/b3 或 VIX>=25)」——**根本没有 200DMA**。
   SKILL.md 里那半句是 policy 落地（2026-08-22）之前旧口径的残留。
   ⇒ 按本节定，**同时也是回到用户的配置意图本身**，不是新增限制。

📌 **200DMA 仍有两处正当用途，字段不废弃**：
- `technical.dev_200dma` 作为**位置描述**写进逐股研究（「现价在 200DMA 上方 x%」），
  它描述位置，**不构成扳机**；
- QQQ 的 `dev_200dma_percentile` 是其估值三重代理之一（§4），那是 **② 估值轴**的输入，
  与 ③ 结构轴无关。

**这条歧义是怎么被发现的**（记以备考）：2026-09-04 GOOGL 恰好压在 200DMA 上
（`dev_200dma` **+0.88%**），两种读法对 ③ 的结论**相反**。当日 ② 以正常化溢价 +21.7%
决定性否决，两读同果 ⇒ 报告**未作裁定**，只把歧义写进诚实边界交由用户定。
**在真正 binding 之前定死，而不是在 binding 那天由报告临场发挥**——这正是本条存在的意义。
